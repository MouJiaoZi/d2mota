export class GameConfig {
    constructor() {
        ...
        GameRules.SetPreGameTime(1); // 进入游戏后号角吹响前的准备时间
        ...
        game.SetAnnouncerDisabled(true); // 是否禁用播音员
        game.SetAnnouncerGameModeAnnounceDisabled(true); // 是否禁用播音员的游戏模式公告
        game.SetKillingSpreeAnnouncerDisabled(true); // 是否禁用击杀连杀播音员
        game.SetDeathTipsDisabled(true); // 是否禁用死亡提示
        game.SetGiveFreeTPOnDeath(false); // 是否在死亡时给予免费的回城卷轴
        game.SetHudCombatEventsDisabled(true); // 是否禁用战斗事件
        game.SetInnateMeleeDamageBlockAmount(0); // 设置近战英雄的固有伤害格挡
        game.SetInnateMeleeDamageBlockPercent(0); // 设置近战英雄的固有伤害格挡百分比
        game.SetInnateMeleeDamageBlockPerLevelAmount(0); // 设置近战英雄的固有伤害格挡每级增加的数值
        game.SetUnseenFogOfWarEnabled(true); // 是否启用未知的战争迷雾
        game.SetFogOfWarDisabled(true); // 是否禁用战争迷雾
        game.SetCameraZRange(11, 3800);
